...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[#0B1020] p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
const SheetContent = React.forwardRef(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-[#0B1020] transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-glow-cyan focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-white/10">
